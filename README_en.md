...
This skill is meant to provide a solid PPT generation workflow. To stay broadly useful, the workflow is a little more complex than most people need every day, and that complexity can sometimes add instability or redundant choices. For example, it supports Codex OAuth member authentication, the local image CLI, and third-party API generation, and it also supports workflows with or without subagents. Most users will eventually use only one of those paths.

After you have a route working, consider asking an AI assistant to edit this skill and lock in your own preferences, such as Codex OAuth, a local third-party provider, a specific API backend, whether to use subagents, output directory conventions, visual styles, or slide pacing. That way you do not need to make the same choices every time.
...
- Works across multiple agents: supports Codex, Claude Code, OpenClaw, Hermes Agent, and other `SKILL.md`-based environments; Codex is the recommended environment because the local image CLI can reuse Codex OAuth/member authentication first.
- Supports multiple image providers: works with Codex OAuth, OpenAI-compatible endpoints, AtlasCloud, `base URL`, and custom model names through one local CLI for `gpt-image-2` or compatible image models.
- Stable staged workflow: confirms the outline, slide count, visual style, and sample slide before full-deck generation; the sample generation step states which image backend was used.
...
> [!TIP]
> You can start using Codex PPT normally to make a deck. In most cases, you do not need to configure the image model by hand; when generating the sample slide, the AI will state the backend it used and guide you through any required configuration if needed.
>
> - If you are logged in to Codex, the local image CLI prefers Codex OAuth/member authentication by default, so you usually do not need an extra API key.
...
Asking for a specific resolution, higher quality, or edits to one slide does not by itself trigger third-party API configuration. If you use Codex through a GPT subscription and local Codex OAuth is available, you can usually use the local image CLI with `--backend auto` and do not need to prepare an API key.
...
4. Generate one sample slide and state which image backend was used.
5. Ask you to approve the sample slide's style, layout rhythm, and text quality.
...
- The default script resolution is 2K 16:9 landscape (`2048x1152`), and it applies to Codex OAuth, local third-party providers, and OpenAI-compatible proxies. If slide images look blurry, especially on text-heavy pages, ask the AI to switch to 4K. Codex subscribers use the local image CLI's Codex OAuth backend by default, reusing local Codex auth against the official images endpoint and writing images directly to the PPT project's local paths.
