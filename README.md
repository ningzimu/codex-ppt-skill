...
这个 skill 主要给大家提供一个还不错的 PPT 生成流程。为了尽量通用，它的流程设计会稍微复杂一些；复杂也会带来不稳定性或者冗余性。比如它同时兼容 Codex OAuth 会员认证、本地图片 CLI、第三方 API 生图，也会兼容有无子 agent 可用这两种情况，但大部分人日常使用时其实只会固定走其中一条路线。

建议大家在走通自己常用的路线之后，让 AI 帮你改一下这个 skill，把你的偏好固定下来，省得每次都重新选择。比如固定使用 Codex OAuth、本地第三方 provider 或某个 API，固定是否使用子 agent，固定常用输出目录、风格、页数节奏等。
...
- 多 agent 可用：支持 Codex、Claude Code、OpenClaw、Hermes Agent 等支持 `SKILL.md` 的环境；最推荐在 Codex 中使用，本地图片 CLI 会优先复用 Codex OAuth/会员认证。
- 多后端生图供应商接入：支持 Codex OAuth、OpenAI 兼容接口、AtlasCloud、`base URL` 和自定义模型名配置，方便通过统一的本地 CLI 使用 `gpt-image-2` 或兼容模型。
- 稳定的阶段化流程：先确认大纲、页数、视觉风格和样张；生成样张时会说明所用生图后端，再进入整套生成，降低一次生成完整 PPT 时的返工和偏航。
...
> [!TIP]
> 你可以先正常使用 Codex PPT 开始制作 PPT。一般不需要自己手动配置生图模型；生成样张时，AI 会说明实际使用的生图后端，并在需要配置时引导你提供相关信息。
>
> - 如果你在 Codex 中已经登录，默认本地图片 CLI 会优先复用 Codex OAuth/会员认证，通常不需要额外配置 API key。
...
指定图片分辨率、提高质量或要求修改某一页，本身不会触发第三方 API 配置。如果你是通过 GPT 会员订阅使用 Codex，并且本机 Codex OAuth 可用，通常可以直接用本地图片 CLI 的 `--backend auto`，不需要准备 API key。
...
4. 生成 1 页样张，并说明本次使用的图片生成后端
5. 让用户确认样张的风格、版式节奏和文字质量
6. 创建 PPT 项目目录
...
- 默认脚本分辨率是 2K 16:9 横屏，适用于 Codex OAuth、本地第三方 provider 和 OpenAI 兼容中转站。如果生成的幻灯片图片比较模糊，尤其是文字较多的页面，可以让 AI 改用 4K 分辨率生成。Codex 会员默认会优先使用本地图片 CLI 的 Codex OAuth 后端，把图片直接写到 PPT 项目的本地路径。
