...
- Stay on the already selected image backend. Do not switch between local CLI backends only because a slide includes source images.
...
- In local CLI mode, pass each required source image with `scripts/image_gen.py edit --image {asset_path}` so the backend receives the image as an input.
- If the user explicitly chooses a UI built-in image tool, every source image must be visible in the conversation context before generating any slide that depends on it. For local image paths, inspect each required image with `view_image` first, then generate or edit the slide.
