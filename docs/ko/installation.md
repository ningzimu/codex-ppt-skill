...
MuAPI는 생성 전용 OpenAI 호환 이미지 엔드포인트를 제공합니다. base URL을 `https://api.muapi.ai/v1`, 모델을 `flux-schnell`로 설정하세요. 어댑터는 문서에 정의된 `model`, `prompt`, `n`, `size`만 전송하고 세 가지 문서 지원 크기를 사용하며, API key를 이미지 주소로 전달하지 않고 HTTPS로 반환된 이미지 URL을 다운로드합니다. 이미지 편집과 지원되지 않는 출력 옵션은 요청 전에 거부됩니다. 자세한 내용은 [MuAPI OpenAI 호환 문서](https://muapi.ai/docs/openai-compatible)를 참고하세요.
